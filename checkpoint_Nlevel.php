...
while($chkpointRow = mysqli_fetch_assoc($chkpointQuery)){
	...
	for($i=0;$i<count($reuseChkArr);$i++){
		...
		if($chkId == $reuseChkId){
			...
			$logic1 = $chkpointRow["Logic"];
			$isDependent1 = $chkpointRow["Dependent"];

			if($logic1 != "" && ($isDependent1 == 1 || $isDependent1 == 5)){
...
function getDependentCpDetail($logic){
	...
	if($logicChkIdString1 != ""){
		...
		while($logicRow = mysqli_fetch_assoc($logicQuery)){
			...
			$logic2 = $logicRow["Logic"];
			$isDependent2 = $logicRow["Dependent"];
			if($logic2 != "" && ($isDependent2 == 1 || $isDependent2 == 5)){
