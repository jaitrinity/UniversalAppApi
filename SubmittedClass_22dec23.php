...
class SubmittedClass{
	...
	function getFilledCheckpoints($obj,$statObj){
		...
		if($approverActId != null && $approverActId != ''){
			$approvedCpSql = "Select r2.*,r1.* from (Select d.ChkId,d.Value as answer from TransactionDTL d where d.ActivityId = '$approverActId' and d.DependChkId = 0 )r1 right join (Select c.* from Checkpoints c where c.CheckpointId in ($approvreCpString)) r2 on (r1.ChkId = r2.CheckpointId)";
			// echo $approvedCpSql;
